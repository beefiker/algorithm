   ...
   **재귀 함수**란 `자기 자신을 다시 호출하는 함수`를 의미한다.
   ...
   - 간단한 예시
     ...
     3. 재귀함수를 이용한 DFS 예제
        ...
        자기 자신을 다시 호출하는 재귀 함수로 깊이 우선 탐색**DFS**을 하는 코드이다.

        처음 dfs함수에 1번 노드를 시작으로, graph[1] 내부의 원소(연결 정보)를 반복문으로 graph[2], graph[7], ∙∙∙ graph[5]까지 연결된 노드들에 대하여 깊이 우선 탐색을 실행하고 탐색한 순서로 출력한다.
